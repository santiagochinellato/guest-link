...
import { signIn } from "next-auth/react"; // For Google Sign In client-side logic if needed, or stick to server action for google?
// Actually, for Google we usually use a form with action={async () => { "use server"; await signIn("google"); }}
// But for separation, let's keep it simple. Auth.js v5 suggests strict server actions or form actions.
    ...
    <div className="flex flex-col gap-6 w-full max-w-[350px] mx-auto">
      <div className="flex flex-col items-center gap-2 text-center mb-2">
        ...
        <p className="text-sm text-gray-500 dark:text-neutral-400">
          Ingresa a tu cuenta para gestionar propiedades
        </p>
      </div>
      ...
      <form action={dispatch} className="flex flex-col gap-4">
        ...
        {errorMessage && (
          <motion.div
            ...
            className="p-3 bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-900/50 rounded-lg text-xs text-red-600 dark:text-red-400"
          >
            {errorMessage}
          </motion.div>
        )}
      ...
      <button
        ...
        // Use a form action instead to keep it robust
        onClick={async () => {
          // This part ideally should be a server action or distinct form
          // But for now client side signIn is fine if session provider is wrapped
