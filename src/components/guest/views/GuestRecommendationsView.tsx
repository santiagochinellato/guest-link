  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-right duration-300">
      <div className="space-y-2">
        <h3 className="text-2xl font-bold text-neutral-900 dark:text-white flex justify-center items-center gap-3 text-center md:text-left">
          <span className="p-2 bg-brand-copper/10 rounded-xl text-brand-copper text-center md:text-left">
            <BookOpen className="w-6 h-6" />
          </span>
          Recomendaciones
        </h3>
        <p className="text-sm text-neutral-500 dark:text-neutral-400 leading-relaxed text-center md:text-left">
      ...
      {/* Categories */}
      <div className="flex gap-2 overflow-x-auto pb-4 mx-4 px-6 no-scrollbar snap-x">
        {categories.length > 0 ? (
          categories.map((cat) => (
            <button
              key={cat}
              ...
              className={cn(
                ...
                activeCategory === cat
                  ...
                  : "bg-white dark:bg-brand-void text-neutral-600 dark:text-gray-300 border-gray-100 dark:border-neutral-700 hover:border-brand-copper/50 hover:bg-gray-50",
              )}
            >
      ...
      <div className="grid gap-6">
        {recommendations
          ...
          .map((place: any, i: number) => (
            <div
              key={i}
              className="group bg-white dark:bg-brand-void rounded-[1.5rem] overflow-hidden shadow-sm border border-gray-100 dark:border-neutral-700 hover:shadow-xl hover:-translate-y-1 transition-all duration-300"
            >
              <div className="p-5 space-y-4">
                <div className="flex justify-between items-start gap-4">
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-brand-copper text-white text-[10px] font-bold uppercase tracking-wider">
